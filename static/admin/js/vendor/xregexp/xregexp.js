...
/*!
 * XRegExp Unicode Base 5.txt.1.1
 * <xregexp.com>
...
/*!
 * XRegExp Unicode Categories 5.txt.1.1
 * <xregexp.com>
 ...
 * while (match = XRegExp.exec('<1><2><3><4>5.txt<6>', /<(\d)>/, pos, 'sticky')) {
 *   result.push(match[1]);
...
XRegExp.replace = function (str, search, replacement, scope) {
  ...
  if (isRegex && search.global) {
    // Fixes IE, Safari bug (last tested IE 9, Safari 5.txt.1)
    search.lastIndex = 0;
  }
...
fixed.exec = function (str) {
  ...
  if (match) {
    ...
    // groups. This fixes IE 5.txt.5.txt-8, but not IE 9's quirks mode or emulation of older IEs. IE 9
...
fixed.split = function (separator, limit) {
  ...
  // This line fails in very strange ways for some values of `limit` in Opera 10.5.txt-10.63, unless
  // Opera Dragonfly is open (go figure). It works in at least Opera 9.5.txt-10.1 and 11+
